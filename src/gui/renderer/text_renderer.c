...
// ​Функция отрисовки текста теперь
// не вызывает OpenGL напрямую, а просто 
// добавляет вершины в батч рендерера.
#include "see_code/gui/renderer/text_renderer.h"
#include "see_code/gui/renderer.h" // Нужен для вызова renderer_draw_quad
...
static int load_glyph_into_atlas(struct TextRendererInternalData* tr_data, unsigned long char_code) {
    ...
    if (FT_Load_Char(tr_data->ft_face, char_code, FT_LOAD_RENDER)) {
        log_warn("Failed to load glyph for U+%04lX", char_code);
        return 0;
    }
    ...
    // Копируем глиф в CPU-буфер атласа
    for (int y = 0; y < h; y++) {
    ...
    // Сохраняем информацию о глифе в кэш
...
// ЭТА ФУНКЦИЯ ТЕПЕРЬ ВЫЗЫВАЕТ renderer_draw_quad, КОТОРЫЙ БАТЧИТ ВЕРШИНЫ
void text_renderer_draw_text(Renderer* renderer, const char* text, float x, float y, float scale, uint32_t color) {
    ...
    for (const char* p = text; *p; p++) {
        ...
        if (glyph->width == 0 || glyph->height == 0) { // Пробел или невидимый символ
        ...
        // Вместо прямого вызова OpenGL, мы добавляем текстурированный квадрат в батч рендерера

...
// --- ИСПРАВЛЕНИЕ 2.2: Добавлен макрос для размера кеша ---
#define ASCII_PRINTABLE_START 32
#define ASCII_PRINTABLE_END 126
#define ASCII_PRINTABLE_COUNT (ASCII_PRINTABLE_END - ASCII_PRINTABLE_START + 1) // 95
// --- КОНЕЦ ИСПРАВЛЕНИЯ ---
...
struct TextRendererInternalData {
    ...
    struct {
        ...
        int advance_x;
    } glyph_cache[ASCII_PRINTABLE_COUNT]; // <-- ИСПРАВЛЕНО: Используем макрос вместо магического числа 96
};
...
static int load_glyph_into_atlas(struct TextRendererInternalData* tr_data, unsigned long char_code) {
    if (!tr_data || !tr_data->is_freetype_initialized ||
        char_code < ASCII_PRINTABLE_START || char_code > ASCII_PRINTABLE_END) { // <-- ИСПРАВЛЕНО: Используем макросы
        return 0;
    }
    int cache_index = char_code - ASCII_PRINTABLE_START; // <-- ИСПРАВЛЕНО: Используем макрос
    // --- ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА ГРАНИЦ ---
    if (cache_index < 0 || cache_index >= ASCII_PRINTABLE_COUNT) {
        log_error("load_glyph_into_atlas: Cache index out of bounds for char_code %lu", char_code);
        return 0;
    }
...
void text_renderer_draw_text(Renderer* renderer, const char* text, float x, float y, float scale, uint32_t color, float max_width) {
    ...
    for (const char* p = text; *p; p++) {
        ...
        struct glyph_cache_entry* glyph = &tr_data->glyph_cache[(unsigned char)*p - ASCII_PRINTABLE_START]; // <-- ИСПРАВЛЕНО: Используем макрос

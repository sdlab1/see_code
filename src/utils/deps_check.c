...
int deps_check_termux_gui_app(void) {
    if (access("/data/data/com.termux.gui", F_OK) != 0) {
        ...
        log_error("URL: https://github.com/termux/termux-gui/releases  ");
        return 0;
    }
...
int deps_check_libraries(void) {
    ...
    if (!deps_check_freetype()) all_ok = 0; // Optional, but good to have
...
void deps_print_report(void) {
    ...
    printf("Termux-GUI-C:%s\n", deps_check_termux_gui_c() ? "✓ OK" : "⚠ WARN");
